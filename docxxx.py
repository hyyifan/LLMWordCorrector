...
class DocxReview:
    def __init__(self, doc_path, key, split_length=1000, max_retries=3, retry_delay=5, max_workers=5):
        ...
        self.input_price_per_1k = 0.0008  # ¥/1000 tokens
        self.output_price_per_1k = 0.002  # ¥/1000 tokens
    ...
    def chat_single(self, item):
        for attempt in range(self.max_retries):
            try:
                ...
                - Workflow:
                1. 依据句子分隔符号(。！？；等)和分段符号"\n",将文本分割为独立的句子。禁止一次分析太长的句子。
                2. 逐句审查，识别错别字或用词不当。
                3. 根据上下文综合判断句子所有词汇的恰当性。
                4. 对存在错误的词语、句子,请列出原句，并标记存在错误，提供修改意见。
                5. 对不存在错误的词语，请你直接忽略该句子，开始审阅下一个句子。
                6. 确保输出中不夹杂任何XML标签。
                7. 确保输出样式不添加任意符号（包括#*-）
                8. 确保输出中不夹在任何本promopt的内容。
                9. 请严格根据examples的json格式进行输出，json格式要求的双引号、冒号、逗号必须半角符号"":,
                - Examples:
                - 例子1:
                ...
                """
                user_content = f'请分析以下内容是否有错字、错词、错句：{query}'
                
                # 估算输入token
                input_tokens = self.estimate_tokens(system_content) + self.estimate_tokens(user_content)
                ...
                # 估算输出token
                output_tokens = self.estimate_tokens(completion.choices[0].message.content)
    ...
    def run(self):
        ...
        print(f"【Token使用情况】")
        print(f"估算的输入token数量: {self.total_input_tokens}")
        print(f"估算的输出token数量: {self.total_output_tokens}")
        print(f"估算的总token消耗量: {self.total_input_tokens + self.total_output_tokens}")
        print(f"【费用估算】")
        print(f"输入token费用: {input_cost:.4f}元")
        print(f"输出token费用: {output_cost:.4f}元")
